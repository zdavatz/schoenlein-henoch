...
pub static DOKUMENT: &[Block] = &[
    ...
    Lead {
        ...
        blocks: &[
            ...
            P(&[
                B("Und der Stand vom 1. September, mit dem Guten zuerst."), T(" Der Wind geht wieder ab, der Appetit kommt ein wenig zurück, und der Stuhl bewegt sich – nach Wochen des Stillstands drei Zeichen in dieselbe Richtung. Nur ist er "), B("grün und schwarz und flüssig"), T(". Schwarz kommt von oben und lässt die Frist für die Magenspiegelung unverändert stehen; grün kommt von unten und wirft eine Frage auf, die auf diesem Blatt bisher fehlte – Antibiotikum, Spitalaufenthalt, 84 Jahre und Säureblocker sind zusammen die Liste, auf der Clostridioides difficile steht, und die Stuhlprobe dafür ist heute verfügbar. Siehe den Abschnitt «Dunkler Stuhl». Und Novalgin läuft jetzt als Infusion – das einzige Schmerzmittel auf ihrer Liste, das den Darm nicht bremst."),
            ]),
            P(&[
    ...
    P(&[T("Diese Frage kommt nicht aus einer Vermutung, sondern daraus, dass die Liste der Risikofaktoren sich hier liest wie ihre Krankengeschichte. Als "), L("bedeutsamster Risikofaktor", "https://www.ncbi.nlm.nih.gov/books/NBK431054/"), T(" gilt ein breit wirksames Antibiotikum, ausdrücklich auch aus der Gruppe der Penicilline – Co-Amoxicillin gehört dazu. Dazu kommen als weitere Risikofaktoren: höheres Lebensalter, ein Spitalaufenthalt, eine längere Aufenthaltsdauer, Begleiterkrankungen und Protonenpumpenhemmer. Sie erfüllt jeden einzelnen davon.")]),
    P(&[B("Wie gross der Beitrag der beiden Mittel ist, lässt sich beziffern."), T(" In einer schwedischen Untersuchung mit 43'152 Erkrankten und 355'172 Vergleichspersonen lag das Risiko nach Antibiotikum "), B("und"), T(" Säureblocker in den letzten dreissig Tagen bei einem "), L("Chancenverhältnis von 17,5", "https://pmc.ncbi.nlm.nih.gov/articles/PMC10904719/"), T(" – für das Antibiotikum allein bei 15,4, für den Säureblocker allein bei 2,7. "), B("Die Zahlen sagen deutlich, wo das Gewicht liegt:"), T(" beim Antibiotikum. Der Säureblocker verschiebt das Ganze noch etwas, aber er ist nicht der Grund.")]),
    ...
    P(&[B("Der Punkt, der mir am wichtigsten erscheint, betrifft aber die Agranulozytose – und zwar auf eine Weise, die vorher nicht galt."), T(" Der Abschnitt oben nennt sie als die gefürchtete Nebenwirkung und verlangt das Differentialblutbild. Die Fachinformation fügt einen Satz hinzu, der jetzt zutrifft: Bei Patienten, die eine "), B("Antibiotikabehandlung"), T(" erhalten, können die Symptome einer neu auftretenden Agranulozytose "), L("verschleiert werden", "https://ch.oddb.org/de/gcc/fachinfo/reg/16952/chapter/restrictions"), T(" – ebenso, wenn Metamizol gegen Fieber gegeben wird. Sie hat zwei Tage vor Eintritt Co-Amoxicillin bekommen, und ihr CRP steigt. "), B("Fieber und Entzündungszeichen sind bei ihr also mehrdeutig geworden:"), T(" Sie können vom Grundleiden kommen, vom Antibiotikum überdeckt sein oder das erste Zeichen einer Blutbildstörung sein. Auseinanderhalten lässt sich das nur mit dem Differentialblutbild, und das steht in der Abklärungstabelle dieses Blattes seit der ersten Fassung.")]),
    ...
    H2("Die neun Fragen, auf die es jetzt ankommt"),
